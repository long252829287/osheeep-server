...
On 2026-07-20, the explicit guarded `DinnerCustomRecipeMySqlIT` run passed 1/1 on the dedicated local MySQL 8.0 catalog. Flyway validated all six source migrations; that catalog started at schema version 4, so this run applied V5 and V6 and finished at version 6. This is not evidence of a fresh V1-V6 migration, a production migration, deployment, or release. Ordinary `mvn test` does not select `*IT`; this evidence requires the explicit `-Dtest=DinnerCustomRecipeMySqlIT -Dspring.profiles.active=local` command. Before Spring creates the data source or Flyway runs, an early fail-fast initializer treats the two raw process environment values read through `System.getenv` as a non-Spring trust anchor: it requires the effective selected/test names and both raw names to be nonblank and exactly equal, requires the `local` profile and an exactly parsed matching MySQL path catalog from the effective `spring.datasource.url`, and rejects a separate `spring.flyway.url`. This prevents higher-priority Spring or JVM system properties from moving all effective settings together to another catalog. Immediately before migration, a test-only `FlywayMigrationStrategy` takes its expected database directly from the raw process environment, opens Flyway's actual data source, requires `SELECT DATABASE()` and every effective Flyway default/configured schema to equal that database, and only then calls `migrate()`. Before API writes and cleanup, the test independently repeats the `SELECT DATABASE()` comparison.
