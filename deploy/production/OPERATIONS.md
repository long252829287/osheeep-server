...
2026-07-15 已确认以下四个固定名称资源存在并完成绑定；同日根据管理员对当前低使用量阶段的
取舍，停止外部拨测，仅保留不依赖 CAT 试用的主机基础监控：

- CAT 任务 `小家开饭-生产健康检查`：GET
  `https://www.osheeep.com/healthz`，每 5 分钟一次，北京和深圳共 2 个节点；要求
  HTTP `200` 且正文包含 `UP`。任务现为“任务暂停”，不再产生拨测请求。
- CAT 策略 `小家开饭-健康检查失败`：正确率 `< 100%`，连续 2 个数据点触发，
  只筛选上述健康检查目标。策略已停用，避免任务暂停后的无数据或误告警。
- 轻量应用服务器策略 `小家开饭-CVM基础故障`：这只是经管理员授权保留固定名称的
  最小等价策略，不是 CVM 事件告警。它只绑定 1 台生产轻量实例，CPU、内存、磁盘
  利用率分别 `> 95%` 且连续 5 个 1 分钟数据点满足时告警；该策略仍启用。
- 通知模板 `小家开饭-生产告警邮件`：触发与恢复通知均开启，周一至周日全天生效，
  仅使用邮件渠道，并保持绑定上述两个策略。当前只有仍启用的轻量实例策略会触发通知。
  接收人是 1 个已有且已验证的联系人；手册不记录邮箱全文。

腾讯云通知模板列表、详情和编辑页都没有测试发送入口，管理员已接受不做测试收件。不得
声称测试邮件已发送或收到；首次真实触发和首次真实恢复时再分别核对邮件是否送达。不得为
制造告警而停止、重启或扰动生产服务。

CAT 为试用能力，管理员未授权付费升级，并于 2026-07-15 主动暂停任务、停用对应策略。
当前没有外部可用性探测；公网 `/healthz` 仍保留且可供人工检查，这一调整不影响业务功能。
轻量实例的 CPU、内存、磁盘策略和邮件模板不依赖 CAT 试用。用户量或可用性要求提升时，
再重新启用 CAT 或配置其他外部探测；首次真实主机告警触发和恢复时仍需核对邮件送达。
